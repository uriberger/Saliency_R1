...
| `--overlap_merge_boxes` | off that ladder: per-step grounding KEPT, and every step scored against the union of the whole chain's boxes — the arm that moves the target without moving the cost. The only mask source whose `w_overlap` transfers (sd ratio 1.02), but merging only grows the mask and 0–68% of completions saturate the grid, tracking chain length at Spearman +0.991. `docs/merged-boxes.md` |
| `test_merge_boxes_cpu.py` | CPU checks for it: the call count is still the incumbent's (3 where `--chain-boxes` costs 1), the mask is the hand-built merged union, an ungroundable step is carried by its neighbours, and `merged_cover` is recorded for the saturated completions the reward drops |
